...
class PostModelTest(TestCase):
    ...
    def test_post_ordering(self):
        post1 = Post.objects.create(owner=self.user, title="Post 1", content="Content 1")
        post2 = Post.objects.create(owner=self.user, title="Post 2", content="Content 2")
...
class IndexViewTest(TestCase):
    def setUp(self):
        ...
        for i in range(6):
            Post.objects.create(owner=self.user, title=f"Post {i}", content=f"Content {i}")
    ...
    def test_pagination(self):
        response = self.client.get(reverse('blog:index'))
        self.assertEqual(len(response.context['page_obj']), 5)  # 5 posts per page
